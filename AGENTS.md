...
**Outputs:** 4 OCI desktop images (snow, snowfield, flurry, and sundog — flurry is the Hyprland/Omarchy-replica desktop, while sundog is the bootc-only KDE Plasma desktop; see docs/plans/2026-08-25-flurry-omarchy-plan.md and docs/adr/0014-name-kde-bootc-product-sundog.md), 1 OCI server image (cayo), and 26 sysext overlay images (1password, 1password-cli, azurevpn, bitwarden, chatgpt, claude-desktop, code-server, coder, debdev, dev, docker, edge, github-copilot, incus, k3s, lemonade, localsend, moonlight, nix, obsidian, paseo, pilothouse, podman, sunshine, tailscale, vscode).
...
just flurry             # Build flurry Hyprland desktop image
...
**Bootc secure composition (Task 4, 2026-07-28):** all five bootc profiles
(`cayo`, `snow`, `snowfield`, `flurry`, and `sundog`) include
`shared/bootc-secure/mkosi.conf`; native A/B profiles never
...
The successful historical three-profile mechanics publication run
`31150007630` proves repository-token write access to the cayo, snow, and
snowfield packages; Flurry and Sundog extend the same exact-repository
publication contract. A long-lived GHCR PAT is neither required nor permitted.
Pinned Cosign v2.6.1 receives
...
- `mkosi.profiles/` defines transport+kernel selector variants (bootc: snow, snowfield, flurry, sundog, cayo; native A/B: cayo-ab-raw, cayo-ab, snow-ab, snowfield-ab — flurry and sundog are bootc-only; the app-bundling "loaded" variants were retired 2026-07 in favor of sysexts)
