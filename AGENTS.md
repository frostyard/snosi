...
**Outputs:** 3 OCI desktop images (snow, snowfield, and sundog — sundog is the bootc-only KDE Plasma desktop, see docs/adr/0016-name-kde-bootc-product-sundog.md), 1 OCI server image (floe), and 27 sysext overlay images (1password, 1password-cli, bitwarden, chatgpt, claude-desktop, code-server, coder, debdev, dev, docker, edge, github-copilot, himmelblau, incus, k3s, lemonade, localsend, moonlight, nix, obsidian, paseo, pilothouse, podman, sunshine, tailscale, voxtype, vscode).
...
just sundog             # Build sundog KDE Plasma desktop image (bootc only)
...
**Bootc secure composition (Task 4, 2026-07-28):** all four bootc profiles
(`floe`, `snow`, `snowfield`, and `sundog`) include
`shared/bootc-secure/mkosi.conf`; native A/B profiles never include it. The fragment uses an isolated low-priority Forky APT sandbox and
...
and the lab `run-firn-install-tests` matrix own fresh-install proof for floe,
...
`sigstoreSigned` scopes only for `ghcr.io/frostyard/cayo`, `floe`, `snow`,
`snowfield`, and `sundog`; each
...
fail-closed rootfs proof: it requires `output/floe`, `bootc`, `ukify`,
...
bootc binary in the temporary floe OCI image (observed 1.16.3), not the host
...
- `mkosi.profiles/` defines transport+kernel selector variants (bootc: snow, snowfield, sundog, floe; native A/B: floe-ab-raw, floe-ab, snow-ab, snowfield-ab — sundog is bootc-only; the app-bundling "loaded" variants were retired 2026-07 in favor of sysexts; floe-ab-raw is the permanent, never-published native A/B dev fixture, and floe-ab/snow-ab/snowfield-ab are the production native A/B profiles, see below)
...
(`floe-ab`, `snow-ab`, `snowfield-ab`), the installer ISO (Phase 8), the
...
base-built (incus needs its qemu GUI libs on floe); content-only rebases
...
but base-built graphical sysexts cannot strip it because floe has no graphical
...
- `native-retention.yml` - Nightly (05:15 UTC, plus manual dispatch with `execute`/`dry-run` and a `keep_previous` override) application of `docs/native-ab-contracts.md` §13 to R2: one matrix job per native namespace (`floe`, `snow`, `snowfield`, installer ISO) calling `shared/native-ab/publish/retention.sh`, which gpgv-verifies the live index before trusting it, keeps current + previous 2 versions, anything newer than the live index, and anything under 24h old, and deletes the rest plus stale `.candidate/` scratch and orphaned `.history/` pairs. Read-only `contents`, repo-level `NATIVE_R2_*` secrets only, never a signing key. Fixture test: `test/native-ab-retention-test.sh`.
