...
- A new "Step 3c" in the harness itself, run on first boot right after the
  ...
  %:.builtin_trusted_keys` holds an `asymmetric` module-signing key;
...
unlike the backports kernel `cayo-ab`/`snow-ab` use, which evidently DOES
...
4. Verify TPM enrollment (`systemd-cryptenroll --tpm2-device=auto
   --tpm2-pcrs= --tpm2-public-key=.snosi-private/pcr-signing.pub
   --tpm2-public-key-pcrs=11`, matching `enroll_token` in
   `test/native-ab-secure-rotation-test.sh`) and confirm unattended reboot
   auto-unlocks `/var` with zero prompts.
5. Confirm hardware function under the installed, Secure-Boot-enforced,
   ...
   controller, wifi/networking, and power management (suspend/resume,
   battery status) all work — this is the one thing no QEMU run can prove.
