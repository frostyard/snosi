...
**Outputs:** 2 OCI desktop images (snow, snowfield), 1 OCI server image (cayo), and 13 sysext overlay images (1password-cli, azurevpn, bitwarden, code-server, debdev, dev, docker, edge, incus, nix, podman, tailscale, vscode).
...
- Inline CI/build pins that cannot live in `checksums.json` can also be tracked by `check-dependencies.yml`; validate upstream values before using `sed` to edit source files
...
- `check-dependencies.yml` - Weekly check for external dependency updates, creates PRs with updated checksums and inline CI/tool pins (Rust toolchain, chunkah digest, Syft, Cosign)
