# 0016 — Name the KDE bootc product Sundog
...
presentation, Discover exposes only its Flatpak backend (Debian bundles the PackageKit
backend plugin in `plasma-discover` itself, so the build strips it), and Snow's tuned,
