...
audit's own unclassified/stale listings from the first real build.)
...
mkosi.images/               # Image definitions (base + 23 sysexts)
...
All snow/snowfield images (bootc AND native A/B) ship a Snow-branded
plymouth theme; the native A/B desktop images additionally gained the kargs
that make the graphical splash actually render (bootc always had `rhgb
quiet` injected by bootc at install time — that was the entire gap, not
missing plymouth packages).

**Theme mechanics** (`shared/snow/tree`): `usr/share/plymouth/themes/snow/
snow.plymouth` is a two-step config using bgrt's cross-theme trick —
`ImageDir=/usr/share/plymouth/themes/spinner` reuses Debian's throbber and
password-dialog assets; our only binary is `usr/share/plymouth/themes/
spinner/watermark.png` (the FirstSetup flower SVG rendered at 256 px via
GdkPixbuf), injected into spinner's directory because the two-step plugin
loads `watermark.png` from `ImageDir` and no deb owns that path.
`etc/plymouth/plymouthd.conf` sets `Theme=snow`. Delivery is ExtraTrees:
the shipping initrd is generated by the dracut PostInstallationScript
(after ExtraTrees), so no skeleton is needed — and a SkeletonTrees copy of
`plymouthd.conf` is actively fatal: it is a dpkg conffile, dpkg prompts
about the pre-existing copy at plymouth's install, and mkosi's
...
1. `splash plymouth.ignore-serial-consoles` (composition/snow): with
   ...
   consoles detected, managing them with details forced" and never probes
   DRM at all — `splash` alone changes nothing, `plymouth.force-splash`
...
2. Native initrds omit the plymouth dracut module (`omit_dracutmodules+="
   ...
   Debian's kernel ships no simpledrm to provide an instant DRM device.
   The real root's `plymouth-start.service` (statically wanted by
   sysinit.target in the plymouth deb) starts long after DRM is up:
   deterministic. bootc initrds keep plymouth (base dracut conf unchanged;
   static test enforces the split).
3. `console=tty0` appended by the desktop channels
   (`shared/native-ab/channels/{snow,snowfield}/mkosi.conf`) — the channel
   is the only desktop fragment `[Include]`d after ab-root, so tty0 is the
   LAST `console=` and wins `/dev/console`. With a serial `/dev/console`
   that `ignore-serial-consoles` then ignores, plymouthd's local terminal
   is NULL and the DRM renderer's input path segfaults
   (`ply_terminal_set_disabled_input`, Ubuntu LP#2103533; NULL guards
   first in plymouth 26.134.222, absent from every Debian suite including
   sid's 24.004.60-5.2). The same upstream bug fires on any MULTI-GPU
...
harness's console pump needs for the first-boot LUKS recovery passphrase.
...
status output moved to tty0 — note for `--full-window` debugging that
"Entering emergency mode" console markers also follow `/dev/console`.

**Validation:** snow-ab secure-boot harness 71/71 post-change (first-boot
passphrase typed through the new serial agent, TPM auto-unlock, GDM,
...
starts. `quiet` was explicitly declined during design on a premise
`console=tty0` later invalidated — revisit deliberately.

Design/spec: `docs/superpowers/specs/2026-07-17-plymouth-native-ab-design.md`
(3 amendments document the falsified assumptions in order).
