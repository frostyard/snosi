...
- Upstream's CPack Depends (`cmake/packaging/linux.cmake`, Qt5 branch of `TRAY_QT_VERSION`) still lists `libqt5widgets5, libqt5svg5`, which dpkg would enforce (~35 MB of never-loaded Qt5 in the delta). The postinst unpacks the deb with `dpkg-deb -R`, strips exactly that pair from `DEBIAN/control`, rebuilds, runs `assert_deb_dependencies_satisfied`, then installs. It fails closed when the pair is absent — an upstream packaging fix must remove the workaround deliberately. The rebuild also has to `chmod 0755` the maintainer scripts first: upstream has always shipped them mode 0644, which `dpkg -i` tolerates but `dpkg-deb -b` refuses. `test/sunshine-capabilities-test.sh` fixtures the strip and the fail-closed branch with real `dpkg-deb`/`dpkg-checkbuilddeps`
