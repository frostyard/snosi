...
- `himmelblaud` runs with no domain configured (verified in a Trixie container: the domain is taken from the first UPN that signs in), so the daemons start at merge time via `multi-user.target.d/10-himmelblau.conf` `Upholds=himmelblau-sysext-setup.service himmelblaud.service`; `himmelblaud.service` itself `Wants=` the HSM PIN init and `Upholds=` the tasks daemon
