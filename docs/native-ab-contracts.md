...
`.efi`, `.disk.raw[.xz]`, `.manifest.json`, and `.sbom.spdx.json` names above
plus an unsigned `SHA256SUMS` (signing is the Phase 7 promotion step, §7) and
a `publication-info.json` pipeline record. `.sbom.spdx.json` (Phase 7) is
