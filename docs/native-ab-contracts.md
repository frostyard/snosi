...
`keyctl show %:.builtin_trusted_keys`. A signed in-tree module (`isofs`)
...
build-time "raise lockdown when EFI Secure Boot is on" wiring, unlike the
backports kernel `cayo-ab`/`snow-ab` use — under enforced SB,
...
production profiles, not just snowfield-ab.
