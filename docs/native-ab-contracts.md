...
<channel>_<version>.features.json
```
...
Phase 7 promotion step, §7) and a `publication-info.json` pipeline record.
`.features.json` is the product-curated sysext feature catalog (proto 1:
`{proto, product, features: [{name, description, documentation, default}]}`),
generated at image build time by `shared/outformat/ab-root/finalize/
features-catalog.finalize` from the image's own `/usr/lib/sysupdate.*.d/
*.feature` files (also written in-image at `/usr/share/snosi/features.json`).
Curation: a `.feature` file may carry `X-Snosi-Products=` (comma-separated
bare product names; absent = all products) and is excluded from catalogs of
products not listed -- the key is ignored cleanly by systemd-sysupdate 261
(zero warnings at debug level, verified live) and updex >= 1.3.0. Installers
fetch the catalog hash-verified via the signed index
(`snosi-install --print-features`) to present features as a checkbox list;
releases predating the catalog simply have no index entry and installers
fall back to manual feature entry. `.sbom.spdx.json` (Phase 7) is
generated by `shared/native-ab/publish/generate-sbom.sh` directly from the
same mkosi package manifest -- a real SPDX 2.3 JSON document, no external
scanner tool or network access required. Exercised statically (no root, no
image build) by `test/native-publish-test.sh`.

The Phase 7 publication pipeline (candidate upload, remote verification,
signed promotion, withdrawal) lives in `shared/native-ab/publish/publish-candidate.sh`,
`verify-remote.sh`, `promote.sh`, and `withdraw.sh`; see
`docs/native-ab-publication.md` for the operational runbook.
...
All 23 sysext transfer/feature pairs ship in component-scoped
...
| PCR signing key | Signed PCR 11 policy authorizing per-machine LUKS `/var`. **MUST be RSA-2048** (see note) | Protected signer | `.snosi-private/pcr-signing.key` |
...
**PCR signing key algorithm — RSA-2048, nothing else.** TPM2 signed-PCR-policy
*unlock* makes the initrd load the PCR public key into the TPM
(`Esys_LoadExternal`) and verify the `.pcrsig` signature in the TPM
(`Esys_VerifySignature`). Both steps constrain the algorithm, and RSA-2048 is
the only choice that passes both — established empirically 2026-07-16 on
systemd 261.1-3 + swtpm with a live enroll→sign→unlock of each candidate:

- **RSA-4096 fails at LoadExternal** (`TPM_RC_VALUE`): RSA-4096 is optional in
  the TPM2 spec and unimplemented by swtpm and many hardware TPMs; systemd
  #30546 (literal default exponent 65537 passed unnormalized) compounds this on
  some TPMs. Enrollment succeeds, every auto-unlock fails, on *every* profile
  (not Surface/kernel-specific).
- **ECC (any curve) fails at VerifySignature** (`TPM_RC_SCHEME`): systemd 261's
  `tpm2_policy_authorize()` (`src/shared/tpm2-util.c`) hardcodes
  `.sigAlg = TPM2_ALG_RSASSA` with no ECDSA branch, so the TPM is handed an
  RSA-shaped verification request against an ECC key. Enrollment succeeds
  (enroll skips signature verification), every auto-unlock fails.
- **RSA-2048 with the default exponent 65537 passes end to end**: LoadExternal
  OK, VerifySignature OK, unseal OK. RSA-2048 is also the TPM2 spec's
  mandatory-to-support RSA size. Generate with
  `openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048`.

The MOK key is unaffected — it may stay RSA-4096 because Secure Boot verifies it
in firmware, never in the TPM. Tooling that fingerprints the PCR key must hash
`i2d_PublicKey` DER (RSA → PKCS#1, EC → uncompressed point), matching systemd's
`pubkey_fingerprint()` — keep the tooling algorithm-agnostic even though only
RSA-2048 is deployable today, so a future systemd with an ECDSA branch needs no
test changes.
...
**Snowfield Surface module-trust decision (Phase 6):** in-tree Surface
kernel modules (`linux-image-surface`) do NOT need re-signing with the
Secure Boot/MOK key, and no second (linux-surface-authored) signing
certificate needs enrollment. Empirically confirmed on a real
`snowfield-ab` build under enforced Secure Boot with a Measured, MOK-signed
UKI (`test/native-ab-secure-boot-test.sh` Step 3c): `CONFIG_MODULE_SIG_ALL=y`
signs every in-tree module — core and Surface alike — with the SAME
per-build "Build time autogenerated kernel key"
(`modinfo -F signer`/`-F sig_key` on `isofs.ko` and `surface_aggregator.ko`
report identical values), and that key is present in the booted guest's
`keyctl show %:.builtin_trusted_keys`. Since the Phase 6 review follow-up
(2026-07-15) the harness binds the SPECIFIC certificate, not just "an
asymmetric key exists": it extracts the embedded build-time cert from the
booted UKI's `.linux` payload and asserts the module's `modinfo -F sig_key`
equals the cert SERIAL while the keyring entry hex equals the cert SKID —
these are two structurally different identifiers (PKCS#7 issuer+serial
signer reference vs Subject Key Identifier) that never match each other
textually, a pitfall measured live before the check was written. A signed
in-tree module (`isofs`)
loads successfully; a deliberately unsigned out-of-tree module built
in-guest against `linux-headers-surface` is rejected
(`insmod: ERROR: could not insert module ...: Key was rejected by
service`). This makes the Secure Boot/MOK key table row above accurate for
Surface as shipped: it signs systemd-boot, the UKI, and (so far) zero
modules, because zero have failed trust validation. Phase 8's installer
therefore does not need a Surface-specific certificate enrollment step.
Re-run this decision if `shared/kernel/surface/mkosi.conf` ever switches to
a differently-signed or externally-supplied Surface kernel package.

A related, independent finding from the same investigation: the Surface
kernel compiles `CONFIG_LOCK_DOWN_KERNEL_FORCE_NONE=y` and carries no
build-time "raise lockdown when EFI Secure Boot is on" wiring — confirmed
by grepping the actual shipped `.config` (`boot/config-6.19.8-surface-3`
from the cached `linux-image-6.19.8-surface-3` deb): alongside
`CONFIG_LOCK_DOWN_KERNEL_FORCE_NONE=y` there is no
`CONFIG_LOCK_DOWN_IN_EFI_SECURE_BOOT` line at all — not even the usual
commented-out "is not set" form other unset bools get in this same file
...
turned out to be wired directly to EFI Secure Boot detection independent of
lockdown's reported level (unsigned modules were rejected even at
`lockdown=none`), so this did not change the module-trust decision above,
but it was a real gap in lockdown's other protections (kexec restriction,
...
means `lockdown=integrity` now unconditionally blocks hibernation (S4) —
a real, previously-working end-user feature — as an accepted trade-off.
Suspend/resume (S3) is a different kernel mechanism and is unaffected by
lockdown.
