...
if jq -e --arg key "$ROOT_DIR/cosign.pub" '
    [.transports.docker | keys[]] == ["ghcr.io/frostyard/cayo", "ghcr.io/frostyard/flurry", "ghcr.io/frostyard/snow", "ghcr.io/frostyard/snowfield", "ghcr.io/frostyard/sundog"] and
    [.transports.docker[][]] == [
...
' "$WORK/copied-policy.json" >/dev/null; then pass "copied policy retains exactly the five scoped Cosign rules"; else fail "copied policy retains exactly the five scoped Cosign rules"; fi
