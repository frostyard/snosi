...
One real, currently-unfixed gap the audit surfaced and correctly refused
to paper over: `/var/lib/aspell/*.rws` (compiled English dictionaries) are
generated by the `aspell-autobuildhash` dpkg trigger at package-configure
time and are NOT dpkg-tracked content (`dpkg -L aspell-en` lists only the
static `/usr/lib/aspell/*.alias` files). No tmpfiles rule reproduces them.
They're classified `discard` (accurately describes today's behavior) but
flagged in the map file and here: spell-checking silently loses its
dictionaries on a native install, with no error anywhere. Needs the same
treatment as dpkg below, or a first-boot regeneration unit — not done as
part of this phase.
...
native image: the dpkg symlink's exact relative target, that
...
least one `image-metadata` line, and that none of this introduces a failed
unit.
